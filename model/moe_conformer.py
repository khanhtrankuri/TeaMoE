...
class MoEConformerLayer(nn.Module):
    ...
    def __call__(self, x, group_ids, deterministic: bool = True):
        ...
        for group_idx, expert_group in enumerate(self.expert_groups):
            ...
            # Lấy trung bình hoặc chọn top-k trong nhóm
...
class MoEConformerEncoder(nn.Module):
    ...
    def setup(self):
        # Khởi tạo 8 ExpertGroups trong setup (Flax best practice)
        self.expert_groups = [
            ExpertGroup(config=group_cfg)
            for group_cfg in self.config.group_configs
        ]

        # Pre-create non-MoE conformer layers
        self.pre_layers = [
            ConformerLayer(
            ...
            for _ in range(self.config.moe_start_layer)
        ]
        ...
        self.moe_layers = [
            ...
            for _ in range(self.config.moe_start_layer, self.config.moe_end_layer)
        ]
        ...
        self.post_layers = [
            ...
            for _ in range(self.config.moe_end_layer, self.config.num_layers)
        ]
